...
22. Q: MicroPython 端的 “WebSocket 客户端模块是否可用” 指什么？需要刷固件吗？
   A: 指设备运行环境能否直接 `import` 到可用的 WebSocket 客户端库（例如 `websocket` / `uwebsocket`）。通常不需要刷固件，只要把对应 .py 库文件放到设备文件系统即可；本项目当前 REPL 已确认可 `import websocket` 与 `import uwebsocket`。

23. Q: 为什么 Phase 1 先不做 Flash/TF 断网缓存？KEY2 做什么？
   A: 测试阶段采集数据价值不高，频繁写 Flash/TF 会产生垃圾与磨损；因此 Phase 1 先打通 WebSocket 直连与实时链路，缓存功能默认关闭。KEY2 用于在需要时临时启用“失败重试缓存”（先使用内存队列，后续再接入 Flash/TF 持久化）。

24. Q: KEY2 绑定 GPIO11 有风险吗？
   A: 有。部分 ESP32 模组的 GPIO6~11 可能连接到板载 Flash，不一定可用或不建议使用；建议优先选用如 GPIO27 等空闲脚。本项目模板已将 KEY2 默认改为 GPIO27；如你自行改回 6~11 段位且出现异常重启/无法读取，请立刻换脚位。
